...
class FollowerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FollowerView(uid: uid);
  }
}

class FollowerView extends StatefulWidget {
  final String uid;
  const FollowerView({super.key, required this.uid});
...
class _FollowerViewState extends State<FollowerView> {
  ...
  Future<void> fetchData() async {
    ...
    final result = await service.getFollowers(user.uid);
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar.get(title: "Followers"),
      ...
      body: Center(
        child: Container(
          ...
          child: loading 
              ...
              : followers.isEmpty ? const SizedBox() : _mainSection(context),
        ),
      ),
    );
  }
  ...
  Widget _mainSection(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return PeopleCard(followers[index]);
      },
    );
  }
}
